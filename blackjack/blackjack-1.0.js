...
let suitNames = ["Hearts","Diamonds","Spades","Clubs"];
...
function renderCards() {
    ...
    for(let j = 0; j < suitNames.length; j++)
    {
        ...
        thisSuit.forEach(addSuitName);
        function addSuitName(rec) {
...
function getAceValue() {
    let trueFalse = Math.round ( Math.random() );
    if(trueFalse === 0) {
        return 1;
    } else {
        return 11;
...
function startGame() {
    ...
    let firstCard = getCardValue(newDeck);
    let secondCard = getCardValue(newDeck);
...
function newCard() {
    if (player.isAlive === true && player.hasBlackJack === false) {
        let cardValue = getCardValue(newDeck);
